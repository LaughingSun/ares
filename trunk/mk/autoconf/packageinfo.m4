...
AC_DEFUN([CS_EMIT_PACKAGEINFO],
    ...
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_STRING], [$PACKAGE_STRING], [atomic])
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_BUGREPORT], [$PACKAGE_BUGREPORT], [atomic])
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_LONGNAME], [$PACKAGE_LONGNAME], [atomic])
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_HOMEPAGE], [$PACKAGE_HOMEPAGE], [atomic])
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_COPYRIGHT], [$PACKAGE_COPYRIGHT], [atomic])
    _CS_EMIT_PACKAGEINFO([$1], [PACKAGE_VERSION_LIST],    
        m4_translit(patsubst(AC_PACKAGE_VERSION, [[^0-9.]+.*$]), [.], [ ]))
    ])
