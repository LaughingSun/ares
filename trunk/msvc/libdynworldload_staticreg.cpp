// This file is automatically generated.
#include "cssysdef.h"
#include "csutil/scf.h"

// Put static linking stuff into own section.
// The idea is that this allows the section to be swapped out but not
// swapped in again b/c something else in it was needed.
#if !defined(CS_DEBUG) && defined(CS_COMPILER_MSVC)
#pragma const_seg(".CSmetai")
#pragma comment(linker, "/section:.CSmetai,r")
#pragma code_seg(".CSmeta")
#pragma comment(linker, "/section:.CSmeta,er")
#pragma comment(linker, "/merge:.CSmetai=.CSmeta")
#endif

namespace csStaticPluginInit
{
static char const metainfo_dynworldload[] =
"<?xml version=\"1.0\"?>"
"<!-- dynworldload.csplugin -->"
"<plugin>"
"  <scf>"
"    <classes>"
"      <class>"
"        <name>ares.dynworld.loader.factories</name>"
"        <implementation>DynamicWorldLoader</implementation>"
"        <description>Dynamic World Loader Plugin</description>"
"      </class>"
"    </classes>"
"  </scf>"
"</plugin>"
;
  #ifndef DynamicWorldLoader_FACTORY_REGISTER_DEFINED 
  #define DynamicWorldLoader_FACTORY_REGISTER_DEFINED 
    SCF_DEFINE_FACTORY_FUNC_REGISTRATION(DynamicWorldLoader) 
  #endif

class dynworldload
{
SCF_REGISTER_STATIC_LIBRARY(dynworldload,metainfo_dynworldload)
  #ifndef DynamicWorldLoader_FACTORY_REGISTERED 
  #define DynamicWorldLoader_FACTORY_REGISTERED 
    DynamicWorldLoader_StaticInit DynamicWorldLoader_static_init__; 
  #endif
public:
 dynworldload();
};
dynworldload::dynworldload() {}

}
